...
static void sig_cb(struct io_src_sig *sig, struct signalfd_siginfo *si)
{
	int ret;
	struct my_sig_src *s = ut_container_of(sig, struct my_sig_src, src_sig);

	if (SIGUSR1 == si->ssi_signo) {
		CU_ASSERT(!(STATE_SIGUSR2_RECEIVED & s->state));
		reached_state(&s->state, STATE_SIGUSR1_RECEIVED);
		ret = kill(getpid(), SIGUSR2);
	} else if (SIGUSR2 == si->ssi_signo) {
		CU_ASSERT(STATE_SIGUSR1_RECEIVED & s->state);
		reached_state(&s->state, STATE_SIGUSR2_RECEIVED);
		s->loop = false;
	} else {
		CU_ASSERT(0);
	}
...
static void testSRC_SIG_INIT(void)
{
	...
	CU_ASSERT_EQUAL(ret, 0);
	ret = io_src_sig_init(&s.src_sig, sig_cb, SIGUSR1, SIGUSR2, NULL);
	CU_ASSERT_EQUAL_FATAL(ret, 0);

	ret = io_mon_add_source(&mon, &(s.src_sig.src));
	CU_ASSERT_EQUAL(ret, 0);
	...
	while (loop) {
		...
		loop = STATE_ALL_DONE != s.state;
	}

out:
	/* debriefing */
	CU_ASSERT(s.state & STATE_SIGUSR1_RECEIVED);
	CU_ASSERT(s.state & STATE_SIGUSR2_RECEIVED);

	/* cleanup */
	io_src_sig_clean(&s.src_sig);
	io_mon_clean(&mon);

	/* error use cases */
	ret = io_src_sig_init(NULL, sig_cb, SIGUSR1, SIGUSR2, NULL);
	CU_ASSERT_NOT_EQUAL(ret, 0);
	ret = io_src_sig_init(&s.src_sig, NULL, SIGUSR1, SIGUSR2, NULL);
	CU_ASSERT_NOT_EQUAL(ret, 0);
	ret = io_src_sig_init(&s.src_sig, sig_cb, NULL);
	CU_ASSERT_NOT_EQUAL(ret, 0);
	/* neither SIGSTOP nor SIGKILL can be caught or ignored */
	ret = io_src_sig_init(&s.src_sig, sig_cb, SIGSTOP, NULL);
	CU_ASSERT_NOT_EQUAL(ret, 0);
	ret = io_src_sig_init(&s.src_sig, sig_cb, SIGKILL, NULL);
	CU_ASSERT_NOT_EQUAL(ret, 0);
